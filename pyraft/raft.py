...
class RaftNode(object):
	...
	def select_peer_req(self, timeout):
		sock_peer_map = {}
		for nid, p in self.get_peers().items():
			if p.raft_wait.sock is not None:
	...
	def handle_request(self, p, toks):
		...
		if term is None or prev_term is None or prev_index is None or commit_index is None:
	...
	def do_follower(self):
		...
		for p in peers:
			...
			if msg_list is None or msg_list == []:
				continue
	...
	def process_install_snapshot(self, p):
		...
		if p.index < self.log.start_index() or (diff >= 100 or diff > len(self.data)/10):
			...
			for old_log in old_logs:
				...
				p.raft_req.write(['append_entry', self.term, old_log.term, old_log.index-1, self.commit_index, old_log.ts] + old_log.cmd)
...
def make_default_node(parsed_args):  # redis interface node is default now
	args = parse_default_args(parsed_args)
